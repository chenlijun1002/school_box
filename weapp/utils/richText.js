...
function richTextParse(data) {
  ...
  var nameArr = ['a', 'b', 'code', 'dd', 'div', 'dl', 'dt', 'em', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'i', 'img', 'label', 'li', 'ol', 'p', 'span', 'strong', 'sub', 'sup', 'table', 'tbody', 'td', 'tfoot', 'th', 'thead', 'tr', 'ul', 'br']; //最终结构树

  var tree = []; //初索引

  var index = 0; //匹配到的所有标签

  var tagArr = data.match(/<[^>]*>/gi); //记录标签索引位置

  var indexArr = []; //计算索引位置

  for (var _i = 0; _i < tagArr.length; _i++) {
    //添加索引值
    indexArr.push(data.indexOf(tagArr[_i])); //从字符数据中删除标签

    data = data.replace(tagArr[_i], '');
  } //记录基本的信息值定义


  var cls, alt, src, width, height, step, style;
  var i = 0; //按标签个数进行循环
  ...
  return tree; //将元素添加到tree中

  function sendInfoTree(idx) {
    getStartInfo(tagArr[idx]);
    var label = tagArr[idx].match(/<*([^> ]*)/)[1]; //第一级分支
    ...
    if (label === 'img') {
      ...
      while (i < tagArr.length - 1) {
        obj.children.push(sendInfoTree(i)); //如果标签中间有文本（即索引不一致）
